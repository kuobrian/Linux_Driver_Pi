...
/* Meta Information */
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Brian 4 GNU/Linux");
...
static long watch_var = 0;

/* Static declaration of waitqueues */
...
int thread_function(void* wait_sel) {
    ...
    printk("waitqueue - Thread monitor wq%d finished execution\n", selection);
    return 0;
}

/**
 * @brief Write watch_var device file
 */
...
/**
 * @brief This functionis called, when the module is loaded into the kernel
 */
static int __init my_module_init(void) {
    int t1 = 1, t2 = 2;
    
    /* init dynamically create waitqueue */
    init_waitqueue_head(&wq2);

    printk("waitqueue - Init thread\n");

    /* Register deivce_name */
    ...
    if (kthread_1 != NULL) {
        printk("waitqueue - Thread 1 was create and is running now\n");
    }
    else {
        printk("waitqueue - Thread 1 could not be create\n");
    ...
    if (kthread_2 != NULL) {
        printk("waitqueue - Thread 2 was create and is running now\n");
    }
    else {
        printk("waitqueue - Thread 2 could not be create\n");
        watch_var = 11;
        wake_up(&wq1);
        mdelay(10);
        unregister_chrdev(MYMAJOR, "my_dev_nr");
