...
static int __init ModuleInit(void) {
    ...
    if (kthread_1 != NULL) {
        ...
        printk("kthread - Thread 1 was create and is running now!\n");
    } else {
        printk("kthread - Thread 1 could not be create!\n");
        return -1;
    }
    kthread_2 = kthread_run(thread_function, &t2, "kthread_21");
    if (kthread_2 != NULL) {
        /* Let's start the thread */
        printk("kthread - Thread 2 was create and is running now!\n");
    } else {
        printk("kthread - Thread 2 could not be create!\n");
